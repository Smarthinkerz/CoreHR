...
export async function seedDatabase() {
  ...
  // Normalize legacy/marketing plan names to canonical billing tiers so feature gating resolves correctly.
  await db.execute(sql`UPDATE organizations SET plan = 'enterprise' WHERE plan IN ('ai_intelligence', 'future_of_work', 'elite')`);
  await db.execute(sql`UPDATE organizations SET plan = 'pro' WHERE plan IN ('growth', 'professional', 'business')`);
  await db.execute(sql`UPDATE organizations SET plan = 'free' WHERE plan IN ('starter', 'basic')`);
