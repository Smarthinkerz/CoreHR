...
router.post("/setup", async (req: Request, res: Response) => {
  try {
    ...
    const otpauth = otplib.generateURI({ issuer: ISSUER, label: user.email, secret });
...
router.post("/verify", async (req: Request, res: Response) => {
  try {
    ...
    const { valid } = otplib.verifySync({ token, secret: currentUser.twoFactorSecret });
    if (!valid) {
