...
// NOTE: The real payment webhook is the public, HMAC-verified
// POST /api/tap-payments/webhook (see server/routes/tapPaymentsRoutes.ts).
// A previous POST /api/billing/webhook/tap route was removed: it sat behind
// requireAuth (so Tap could never call it) AND trusted a client-supplied
// metadata.organizationId, letting any authenticated user upgrade ANY org's
// plan — a cross-tenant privilege-escalation hole. Do not reintroduce a
// session-authenticated webhook that mutates an org by client-supplied id.
